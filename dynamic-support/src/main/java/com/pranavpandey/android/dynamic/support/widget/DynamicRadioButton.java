...
/**
 * A RadioButton to apply color filter according to the supplied
 * parameters.
 */
public class DynamicRadioButton extends AppCompatRadioButton implements DynamicWidget {
    ...
    public void setColor() {
        if (mColor != WidgetDefaults.ADS_COLOR_UNKNOWN) {
            ...
            if (DynamicVersionUtils.isLollipop()) {
                setButtonTintList(DynamicResourceUtils.getColorStateList(mColor));
            } else {
                setButtonDrawable(DynamicDrawableUtils.colorizeDrawable(
                        DynamicResourceUtils.getDrawable(getContext(),
                                R.drawable.abc_btn_radio_material), mColor));
            }
        }
    }
}
